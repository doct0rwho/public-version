...
async function getActiveUsers365Days(req, res) {
  try {
    ...
    const results = await db.raw(
      `
      SELECT timeStart AS date, COUNT(DISTINCT userId) AS count
      FROM (
    ...
    const usersByDate = results[0].map((row) => ({
    ...
    for (let i = 0; i < 365; i++) {
      ...
      // console.log(newArr[i].date.toISOString().split("T")[0])
      // console.log(usersByDate[i])
      const result = usersByDate.find(
        (result) =>
...
module.exports.getActiveUsers365Days = getActiveUsers365Days;
