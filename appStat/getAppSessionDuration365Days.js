...
async function getAppSessionDuration365Days(req, res) {
  try {
    ...
    const totalDurationByDate = results[0].map((row) => ({
    ...
    for (let i = 0; i < 365; i++) {
      ...
      const result = totalDurationByDate.find(
        (result) =>
...
module.exports.getAppSessionDuration365Days = getAppSessionDuration365Days;
